...
namespace MxPlot.Extensions.Fft
{
    ...
    /// </summary>
    public static class Fft2DExtension
    {
        #region Setup static delegates and MKL availability flags
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the global options used for Fourier transform operations. FourierOptions.Deafult (symmetrical scaling) is a default option.
        /// </summary>
        public static FourierOptions FourierOptions { get => _fourierOption; set => _fourierOption = value; }

        static Fft2DExtension()
        {
            ...
            if (IsNativeMklAvailable)
            {
                try
                {
                    if (MathNet.Numerics.Control.TryUseNativeMKL())
                    {
                        ...
                        _inverse2D = (array, ynum, xnum) => Fourier.Inverse2D(array, ynum, xnum, _fourierOption);
                        IsNativeMklUsing = true;
                        Trace.WriteLine("[FFTExtensionMkl] Switched to NativeMKL for 2D FFT provider.");
                    }
                }
                catch (Exception ex)
                {
        ...
        private static void ImportData(object src, int idx, Complex[] dst, int h, int w, bool applySwap, bool useParallelSwap)
        {
            if (src is MatrixData<Complex> srcComplex)
            {
                ...
                if (applySwap)
                    ...
                    SwapKernel(srcArr, dst, h, w, SwapDirection.CenterToCorner, useParallelSwap);
                else
            ...
            where T : unmanaged
        {
            ...
            if (!skipScaleSetup && !dst.GetScale().Equals(ftScale))
            {
                //注意必要。もしParallel.Forで複数フレームを同時に処理する場合、スケールの更新はスレッドセーフである必要がある。
                //skipScaleSetupをtrueの場合は、呼び出し元でフレーム全体のスケールを一度だけ設定する
            ...
            try
            {
                ...
                if (!needsBuffer)
                {
                    ...
                    ImportData(src, srcIndex, primary, ynum, xnum, applySwap: false, useParallelSwap: useParallel);
                ...
                else
                {
                    ...
                    ImportData(src, srcIndex, secondary!, ynum, xnum, applySwap: needsPreSwap, useParallelSwap: useParallel);
                    ...
                    {
                        // Corner -> Center
                        SwapKernel(secondary!, primary, ynum, xnum, SwapDirection.CornerToCenter, useParallel);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Performs a 2D Fast Fourier Transform (FFT) on each frame of the source matrix and returns the result as a
        /// complex-valued matrix.
        /// </summary>
        /// <remarks>If the source matrix contains only one frame, the FFT is performed on that frame. For
        /// multi-frame matrices, the FFT is applied independently to each frame in parallel. The frequency domain
        /// scaling is updated in the result to reflect the transformation.</remarks>
        /// <typeparam name="T">The type of the elements in the source matrix. Must be an unmanaged type.</typeparam>
        /// <param name="src">The source matrix containing one or more frames to be transformed.</param>
        /// <param name="isForward">true to perform a forward FFT; false to perform an inverse FFT.</param>
        /// <param name="option">Specifies the frequency shift option to apply during the FFT operation.</param>
        /// <param name="dst">An optional destination matrix to store the FFT results. If null, a new matrix is created. Must have the
                ...
                where T : unmanaged
        {
            ...
            if(dst == null)
            {
                dst = new MatrixData<Complex>(src.XCount, src.YCount, src.FrameCount);
            }
            ...
            try
            {
                switch (option)
                {
                    ...
                    case ShiftOption.Centered:
                        ...
                        SwapKernel(primary, secondary!, ynum, xnum, SwapDirection.CornerToCenter, useParallelSwap);

                        // 4. Action (Secondary: Center DC)
                        action(secondary!, ftScale);

                        // 5. SwapL: Center -> Corner (Secondary -> Primary)
                        SwapKernel(secondary!, primary, ynum, xnum, SwapDirection.CenterToCorner, useParallelSwap);
                    ...
                    case ShiftOption.BothCentered:
                        ...
                        SwapKernel(input, secondary!, ynum, xnum, SwapDirection.CenterToCorner, useParallelSwap);
                        ...
                        SwapKernel(secondary!, primary, ynum, xnum, SwapDirection.CornerToCenter, useParallelSwap);
                        ...
                        SwapKernel(primary, secondary!, ynum, xnum, SwapDirection.CenterToCorner, useParallelSwap);
        ...
        /// </summary>
        private static void SwapKernel(Complex[] srcArray, Complex[] dstArray, int height, int width, SwapDirection dir, bool useParallel = true)
        {
            // 1. シフト量の決定
            ...
            else //Single-threaded procedure (used for Fft2DAllFrames)
            {
                for (int iy = 0; iy < height; iy++)
                {
                    int dstY = (iy + shiftY) % height;
                    ReadOnlySpan<Complex> sRow = srcArray.AsSpan(iy * width, width);
                    Span<Complex> dRow = dstArray.AsSpan(dstY * width, width);
                    int splitPoint = width - shiftX;
                    sRow.Slice(splitPoint, shiftX).CopyTo(dRow.Slice(0, shiftX));
                    sRow.Slice(0, splitPoint).CopyTo(dRow.Slice(shiftX, splitPoint));
                }
            }
