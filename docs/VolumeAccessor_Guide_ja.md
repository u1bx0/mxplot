...
**出力モード・進捗・キャンセル**: `Restack`にはオプションで`LoadingMode outputMode`（既定は
`Auto`で、再配置後のボリュームサイズに基づき`VirtualPolicy`が判定）、`IProgress<int>? progress`、
`CancellationToken`も渡せます。
...
`X`/`Y`（実質的な転置）が`Virtual`出力に解決された場合、書き込みはRAM上限付きの出力フレーム帯
（バンド）をマネージドメモリ上にバッファし、完成したフレームを1つずつMMFへ連続書き込みします。
新規作成した数GB規模のスパースMMFへ行単位で分散書き込みすると壊滅的に遅いことが実測で判明した
ためです。`Z`（元々フレーム順）は常に1フレームずつストリーム書き込みするため、`outputMode`に
関わらずピークメモリは1フレーム分のままです。詳細は`VolumeAccessor.Restack`のXMLドキュメントと
`docs/MatrixData_MethodCallMap.md`を参照してください。
...
**バッファ再利用**: `X`/`Y`では、`SliceAt`にオプションで`IMatrixData? dst`と`int dstIndex`も
渡せます。`dst`の`dstIndex`番目のフレームが十分な大きさなら、スライスはそのバッファへ直接書き
込まれ、戻り値のラッパーもそのバッファを共有します。画面表示中のフレームへ繰り返し再スライス
する呼び出し元（例: カーソル追従する直交Sideビュー）は追加割り当てなしで同じインスタンスを
表示し続けられます。`ViewFrom.Z`ではコピーなしで参照そのものを返すため無視されます。

```csharp
// 初回呼び出し: dstがまだ無いので新規割り当て。
var xzSlice = volume.SliceAt(ViewFrom.Y, 128);

// 以降の呼び出し: xzSlice自身のフレームバッファを再利用 -- 更新毎の割り当てなし。
...
4. **大きなデータはチャンク処理**: `Restack`は完全コピーを作成 -- 非常に大きなボリュームでは
   `LoadingMode.Virtual`を渡す（または`Auto`/`VirtualPolicy`の判定に任せる）ことで、出力全体分
   ではなくバンド1つ分のフレームにピークRAMを抑えられる
...
- `Restack`は出力全体の完全コピーを作成する。
  - `LoadingMode.InMemory`（または`Auto`がこれに解決される場合）: 元データに加えて出力サイズ
    分のマネージドヒープメモリが必要 -- 512×512×100の`float`: 約100MB × 2 = 200MB。
  - `LoadingMode.Virtual`（または`Auto`が大きな出力に対してこれに解決される場合）: 出力はMMF上
    に置かれ、`X`/`Y`ではピークの*マネージド*メモリは出力フレーム帯（バンド）1つ分
    （`GC.GetGCMemoryInfo().TotalAvailableMemoryBytes`の一部）に収まり、出力全体のサイズには
    比例しない -- [VirtualFrames ガイド](VirtualFrames_Guide.md)参照。`Z`は常に1フレームずつ
    ストリーム処理する。
- 大きなボリュームではサブ領域処理を検討
...
```
異なる視点からボリュームを再配置。完全コピーを作成 -- `outputMode`と、`Auto`の場合は
`VirtualPolicy`に応じてインメモリまたはMMF（`Virtual`）に振り分けられる。

```csharp
public MatrixData<T> SliceAt(ViewFrom axis, int index, IMatrixData? dst = null, int dstIndex = 0)
```
指定インデックスで2D断面を抽出。`dst`/`dstIndex`により、新規割り当てせず既存フレームバッファを
再利用できる（`ViewFrom.Z`では無視される）。
...
*最終更新: 2026-08-31  (Generated by GitHub Copilot)*
